...
async function procesarFrame() {
  ...
  // RESOLUCIÓN ALTA PARA OCR (1200px de ancho)
  canvas.width = 1200; 
  canvas.height = 250;

  // RECORTE PRECISO: Sincronizado con el margen del CSS (25vh)
  // sx, sy, sw, sh representan el área dentro del cuadro verde
  ...
  // PRE-PROCESAMIENTO AVANZADO (Aumentar contraste drásticamente)
  let imgData = ctx.getImageData(0, 0, 1200, 250);
  let d = imgData.data;
  for (let i = 0; i < d.length; i += 4) {
    let gray = (d[i] + d[i+1] + d[i+2]) / 3;
    // Umbral más agresivo para eliminar patrones de seguridad del papel
  ...
  try {
    ...
    // REGEX MEJORADO: Busca de 8 a 9 números seguidos de A o B
...
// BOTONES DE CIERRE: RECARGA COMPLETA
