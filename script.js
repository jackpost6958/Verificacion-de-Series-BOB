...
async function initWorker() {
  if (!worker) {
    ...
    await worker.setParameters({ tessedit_char_whitelist: '0123456789AB', tessedit_pageseg_mode: '7' });
  }
}
...
document.getElementById("scanBtn").onclick = async () => {
  try {
    await initWorker();
    streamRef = await navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment", width: { ideal: 1280 } } });
...
document.getElementById("modal-close").onclick = cerrarTodo;
document.getElementById("closeBtn").onclick = cerrarTodo;
